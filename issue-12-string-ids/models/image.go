...
var ImageWhere = struct {
	...
	PostID:      whereHelperstring{field: "`image`.`post_id`"},
...
type imageR struct {
	...
	ImageVariations ImageVariationSlice `boil:"ImageVariations" json:"ImageVariations" toml:"ImageVariations" yaml:"ImageVariations"`
}
...
var (
	...
	imageColumnsWithoutDefault = []string{"id", "post_id", "views", "original_url"}
...
func (imageL) LoadImageVariations(ctx context.Context, e boil.ContextExecutor, singular bool, maybeImage interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`image_variation`),
		qm.WhereIn(`image_variation.image_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
