...
"github.com/web-ridge/gqlgen-sqlboiler-examples/issue-12-string-ids/graphql_models")
		...
		func CommentLikeCreateInputToModelM(
			...
			for key, _ := range input {
				switch key {
					case "commentId":
						modelM[models.CommentLikeColumns.CommentID] = 
						case "likeType":
		...
		func ImageCreateInputToModelM(
			...
			for key, _ := range input {
				switch key {
					case "postId":
						modelM[models.ImageColumns.PostID] = 
						case "views":
		...
		func ImageVariationCreateInputToModelM(
			...
			for key, _ := range input {
				switch key {
					case "imageId":
						modelM[models.ImageVariationColumns.ImageID] = 
						}
			}
			return modelM
		}
		...
		func LikeCreateInputToModelM(
			...
			for key, _ := range input {
				switch key {
					case "postId":
						modelM[models.LikeColumns.PostID] = 
						case "likeType":
